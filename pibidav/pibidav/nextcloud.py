...
""" pibiCo add code for tag mgt """
import json
""" pibiCo end code for tag mgt """
...
class Client(object):
    ...
    """
    -------------------------- ---------------
    |  pibiCo start code for tag management  |
    ------------------------------------------
    """
    def add_tag(self, display_name):
        """Create internal tag in the system
        
        params: display_name for the tag name
        returns: HTTPResponse
        raises: HTTPResponseError in case an HTTP error status was returned
        """
        url_tag = self.url + 'remote.php/dav/systemtags'
        tag = { "name": display_name, "userVisible": "true", "userAssignable": "true" }
        data = json.dumps(tag)
        headers = { "Content-Type": "application/json" }
        res = self._session.post(url_tag, data=data, headers=headers)
    ...
    def list_tags(self):
        """Returns the tag listing for the instance
        ...
        method = 'PROPFIND'
        url_tag = self.url + 'remote.php/dav/systemtags'
        data = '<?xml version="1.0" encoding="utf-8" ?><a:propfind xmlns:a="DAV:" xmlns:oc="http://owncloud.org/ns"><a:prop><oc:display-name/><oc:user-visible/><oc:user-assignable/><oc:id/></a:prop></a:propfind>'
        headers = { "Content-Type": "text/xml" }
        res = self._session.request(method, url_tag, data=data, headers=headers)
    ...
    def assign_tag_to_file(self, fileid, tagid):
        """Assigns a tag to a file in NextCloud
          
          param fileid: fileid for the file or folder
          param tagid: tagid to  assign to the file or folder
          returns: HTTPResponse
          raises: HTTPResponseError
        """
        url_tag = self.url + 'remote.php/dav/systemtags-relations/files/{0}/{1}'.format(str(fileid), str(tagid))
        data = None
        headers = { "Content-Type": "text/xml" }
        res = self._session.put(url_tag, data=data, headers=headers)
    ...
    def get_tag(self, tagid):
        """Get properties from a collaborative tag from its id
         param tagid: tagid for the tag
         returns: HTTPResponse
         raises: HTTPResponseError
        """
        method = 'PROPFIND'
        url_tag = self.url + 'remote.php/dav/systemtags/{}'.format(str(tagid))
        data = '<?xml version="1.0" encoding="utf-8" ?><a:propfind xmlns:a="DAV:" xmlns:oc="http://owncloud.org/ns"><a:prop><oc:display-name/><oc:user-visible/><oc:user-assignable/><oc:id/></a:prop></a:propfind>'
        headers = { "Content-Type": "text/xml" }
        res = self._session.request(method, url_tag, data=data, headers=headers)
    ...
    def delete_tag(self, tagid):
        """Delete a collaborative tag from the NextCloud Server
         param tagid: tagid for the tag
         returns: HTTPResponse
         raises: HTTPResponseError
        """
    ...
    def remove_tag_from_file(self, fileid, tagid):
        """Remove a tag from a file in NextCloud
       
         param fileid: fileid of the file or folder
         param tagid: tagid to remove from file or folder
         returns: HTTPResponse
         raises: HTTPResponseError
        """
        method = 'DELETE'
    ...
    def delete_tags_from_file(self, fileid, taglist):
        """Remove a list of tags from a file in NextCloud
       
         param fileid: fileid of the file or folder
         param taglist: list of tag names ['pibico','test','pibico-test'] to remove from file or folder
         returns: HTTPResponse
         raises: HTTPResponseError
        
        """
        method = 'PROPFIND'
        url_tag = self.url + 'remote.php/dav/systemtags-relations/files/{}'.format(str(fileid))
        data = '<?xml version="1.0" encoding="utf-8" ?><a:propfind xmlns:a="DAV:" xmlns:oc="http://owncloud.org/ns"><a:prop><oc:display-name/><oc:user-visible/><oc:user-assignable/><oc:id/></a:prop></a:propfind>'
        headers = { "Content-Type": "text/xml" }
        res = self._session.request(method, url_tag, data=data, headers=headers, allow_redirects=False)
        if res.status_code >= 400:return ''
        if res.status_code in [200, 207]:
          result = self._parse_dav_response(res)
          for n in result:
            tag_name = n.attributes['{http://owncloud.org/ns}display-name']
            tagid = n.attributes['{http://owncloud.org/ns}id']
            if tag_name in taglist:
              status = self.remove_tag_from_file(fileid, tagid)
              
          return True
        
    """
    ----------------------------------------
    |  pibiCo end code for tag management  |
    ----------------------------------------
    """
            ...
            return ShareInfo(
                                {
                                    ...
                                    'name': data_el.find('name').text if data_el_name ## Modified from pyocclient to fit NextCloud
                                    else ''
                                }
            )
        raise HTTPResponseError(res)
