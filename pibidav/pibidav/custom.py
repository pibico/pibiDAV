...
def upload_file_to_nc(doc, method=None):
  ...
  if dt != 'File' and dn:
    ...
    if document.nc_enable and document.nc_folder:
      ...
      ## Upload file to NC
      frappe.logger().info(f"Uploading file to NC: {nc_path}")
      enqueue(method=nc.put_file,queue='short',timeout=300,now=True,**nc_args)

      ## Verify file was uploaded
      import time
      time.sleep(2)  # Wait for file to be indexed
      try:
        file_info = nc.file_info(nc_path)
        frappe.logger().info(f"File verified in NC: {file_info}")
      except Exception as e:
        frappe.log_error(f"File not found in NC after upload: {str(e)}", "PibiDAV Upload")

      ## Get Shared Link from NC - call directly (not via enqueue)
      share_link = ""
      try:
        frappe.logger().info(f"Attempting to create share for: {nc_path}")
        share = nc.share_file_with_link(path=nc_path)
        frappe.logger().info(f"share_file_with_link returned: {share}")
        if share:
          share_link = share.get_link()
          frappe.logger().info(f"Share link created successfully: {share_link}")
        else:
          frappe.logger().warning(f"share_file_with_link returned None")
      except Exception as e:
        frappe.log_error(f"Error creating share link: {str(e)}", "PibiDAV Share Link")
        frappe.logger().error(f"Full exception details: {type(e).__name__}: {str(e)}")
        share_link = ""
      ...
      frappe.logger().info(f"File doc updated with share_link: {share_link}")
      ...
      return {'fname': doc, 'local_path': local_path, 'local_site': share_link}
  else:
    return
